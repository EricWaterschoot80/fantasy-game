...
const GAME = {
  ...
  ui: {
    ...
    q_readbook: { nl: 'De zon is verduisterd — het gloeiende boek is nu te LEZEN! Tik het aan en ontvang de SPREUK VAN ONZICHTBAARHEID', en: 'The sun is eclipsed — the glowing book can be READ now! Tap it and receive the SPELL OF INVISIBILITY' },
    q_altar:    { nl: 'De onzichtbaarheidsspreuk is binnen! Open nu de geheime deur: kijk tijdens de ZONSVERDUISTERING door de telescoop naar de hemelschijf van de Raaf, en draai de schijven van het DEURZEGEL tot die drie tekens bovenaan staan (1 = buitenste, 2 = middelste, 3 = binnenste). Trek dan aan de hendel', en: 'The invisibility spell is yours! Now open the secret door: during the ECLIPSE, look through the telescope at the Raven’s sky-wheel, then turn the DOOR SEAL discs until those three signs sit at the top (1 = outer, 2 = middle, 3 = inner). Then pull the lever' },
    ...
    q_key:      { nl: 'Je bent onzichtbaar langs de wachter! Pak nu MUISSTIL de gouden sleutel van de wachttafel: schuif de stippen-sleutels één voor één opzij (•, ••, •••) en pauzeer zodat de geluidsmeter niet volloopt', en: 'You slipped past the guard unseen! Now take the golden key from the guard’s table SILENTLY: slide the dotted keys aside one by one (•, ••, •••) and pause so the noise meter doesn’t fill up' },
    q_distract: { nl: 'Je hebt de gouden sleutel! Maar de wachter staart recht naar de cel... Vraag je VADER om raad', en: 'You have the golden key! But the guard stares straight at the cell... Ask your FATHER for advice' },
    q_throwNut: { nl: 'Vader gaf je een WONDERNOOT! Gooi hem in het plasje water links bij de trap', en: 'Father gave you a WONDER-NUT! Toss it into the puddle of water by the stairs, on the left' },
    q_dance:    { nl: 'De wondernoot dobbert in het plasje... Spreek nu de DANS-SPREUK uit (tik hem aan in je tas) en kijk wat er groeit!', en: 'The wonder-nut bobs in the puddle... Now cast the DANCE SPELL (tap it in your bag) and watch what grows!' },
  ...
  items: {
    ...
    cellkey: { name: { nl: 'Gouden Celsleutel', en: 'Golden Cell Key' }, icon: '🗝️', img: 'assets/art/item-cellkey.png', sparkle: true, border: 'gold',
             look: { nl: 'De gouden sleutel van de wachter — muisstil van de bos bevrijd. Hij past op het glinsterende sleutelslot van de celdeur.', en: 'The guard’s golden key — freed from the ring without a sound. It fits the glittering keyhole of the cell door.' } },
    ...
    bignut: { name: { nl: 'Wondernoot', en: 'Wonder-Nut' }, icon: '🌰', img: 'assets/art/item-bignut.png', sparkle: true,
             look: { nl: 'Een grote glanzende noot van je vader. Er zit al een piepklein groen scheutje in — hier wil een plant uit groeien! Hij heeft alleen water nodig... en misschien een dansje.', en: 'A big glossy nut from your father. A tiny green sprout already peeks out — a plant wants to grow from this! It only needs water... and perhaps a little dance.' } },
  ...
  questRules: [
    ...
    { when: { flag: 'nutInPuddle', notFlag: 'guardTurned' },             quest: 'q_dance' },         // noot in het plasje -> dans-spreuk!
    { when: { flag: 'gotNut', notFlag: 'nutInPuddle' },                  quest: 'q_throwNut' },      // wondernoot gekregen -> in het plasje gooien
    { when: { has: 'cellkey', notFlag: 'guardTurned' },                  quest: 'q_distract' },      // sleutel binnen -> vraag vader om raad
    { when: { flag: 'guardPassed', notFlag: 'gotCellKey' },              quest: 'q_key' },           // onzichtbaar langs de wachter -> pak muisstil de gouden sleutel van de tafel
  ...
  scenes: {
    ...
    library: {
      ...
      depth: { far: 210, near: 320, sFar: 0.96, sNear: 1.46 },   // Finn duidelijk groter in de zaal
      walkable: [
      ...
      hotspots: [
        ...
        },
        {
          ...
          look: (state) => state.flags.altarSolved
            ...
            ? { nl: '\u201CKrra! De onzichtbaarheidsspreuk is van jou! Nu de geheime deur: kijk door de TELESCOOP nu de sterren schijnen \u2014 in het hart van de hemelschijf staan drie tekens onder elkaar. Draai de schijven van het Deurzegel tot ze alle drie bovenaan staan \u2014 en trek aan de hendel!\u201D', en: '\u201CCaw! The invisibility spell is yours! Now the secret door: look through the TELESCOPE while the stars shine \u2014 in the heart of the sky-wheel three signs sit one above the other. Turn the Door Seal discs until all three sit at the top \u2014 and pull the lever!\u201D' }
            : state.flags.eclipseActive
            ? { nl: '\u201CKrra! De zon is gedoofd \u2014 het boek is nu te L\u00c9ZEN! Tik het aan: daarin wacht de spreuk van de onzichtbaarheid.\u201D', en: '\u201CCaw! The sun is dark \u2014 the book can be READ now! Tap it: the spell of invisibility waits inside.\u201D' }
        ...
        },
        {
          ...
          walkTo: { x: 300, y: 256 },
          look: (state) => state.flags.altarSolved
            ? { nl: 'Het grote boek rust. De onzichtbaarheidsspreuk staat veilig in jouw toverboek en de geheime deur staat open.', en: 'The great book rests. The invisibility spell sits safely in your spellbook and the secret door stands open.' }
            : state.flags.sawSigns
            ? { nl: 'De onzichtbaarheidsspreuk staat in je toverboek. En die tekening achterin: het sterren-altaar met zon, maan en ster BÓVENAAN — draai de schijven precies zo en trek aan de hendel!', en: 'The invisibility spell is in your spellbook. And that drawing at the back: the star altar with sun, moon and star at the TOP — set the discs just so and pull the lever!' }
            : (state.flags.gotEclipseSpell && state.flags.eclipseActive)
            ? { nl: 'Nu de zon verduisterd is, gloeien de toverletters helder op — het boek is te LEZEN! Tik het aan en ontvang wat erin geschreven staat.', en: 'Now that the sun is darkened, the magic letters glow bright — the book can be READ! Tap it and receive what is written inside.' }
            : state.flags.gotEclipseSpell
            ? { nl: 'Bij daglicht toont het grote boek alleen vage krabbels — alleen bij het licht van een zonsverduistering zijn de toverletters te lezen... Spreek de spreuk uit bij het raam en lees dán het boek.', en: 'In daylight the great book shows only faint scribbles — only by the light of a solar eclipse can the magic letters be read... Cast the spell at the window, then read the book.' }
            : state.flags.wizardTripping
        ...
        },
        {
          ...
          walkTo: { x: 404, y: 252 },
          zoomImg: 'assets/art/telescope-horoscope.jpg',
          ...
          setFlag: 'sawStars',
          look: { nl: 'Een prachtige koperen telescoop, gericht op de hemel boven het grote raam. Op de koperen buis is een kleine raaf gegraveerd.', en: 'A beautiful brass telescope, aimed at the sky above the great window. A little raven is engraved on the brass tube.' }
        },
        {
          id: 'altar',
          ...
          discPuzzle: {
            ...
            hint: { nl: 'Drie draaibare schijven vol sterren-tekens. Tik links of rechts van de buitenste, middelste of binnenste schijf om hem te draaien, tot de juiste COMBINATIE bovenaan onder de wijzer staat \u2014 en trek dan aan de HENDEL. Welke drie tekens? Kijk door de TELESCOOP wanneer de sterren overdag schijnen: het hart van de hemelschijf toont ze van boven naar beneden \u2014 1 = buitenste, 2 = middelste, 3 = binnenste schijf!', en: 'Three turnable discs full of star-signs. Tap left or right of the outer, middle or inner disc to turn it, until the right COMBINATION sits at the top under the pointer \u2014 then pull the LEVER. Which three signs? Look through the TELESCOPE while the stars shine by day: the heart of the sky-wheel shows them top to bottom \u2014 1 = outer, 2 = middle, 3 = inner disc!' },
            setFlag: ['altarSolved'],
            ...
            wrongText: { nl: 'Je trekt aan de hendel... maar het zegel blijft dof. Dit is nog niet de juiste combinatie! (Hint: kijk door de telescoop tijdens de zonsverduistering \u2014 teken 1 bovenaan op de BUITENSTE schijf, teken 2 op de MIDDELSTE, teken 3 op de BINNENSTE.)', en: 'You pull the lever... but the seal stays dull. Not the right combination yet! (Hint: look through the telescope during the eclipse \u2014 sign 1 on top of the OUTER disc, sign 2 on the MIDDLE, sign 3 on the INNER.)' },
            solvedText: { nl: 'De drie tekens van de Raaf gloeien \u00e9\u00e9n voor \u00e9\u00e9n op \u2014 het DEURZEGEL vlamt goudkleurig aan! Met een diep, knarsend gerommel zwaait achter de boekenkast een GEHEIME DEUR open. Daarachter voert een trap omlaag, de duisternis in \u2014 naar de kerker...', en: 'The Raven\u2019s three signs glow one by one \u2014 the DOOR SEAL blazes golden! With a deep grinding rumble a SECRET DOOR swings open behind the bookcase. Beyond it a stair leads down, into the dark \u2014 toward the dungeon...' },
    ...
    dungeon: {
      ...
      walkable: [
        { x: 76, y: 212, w: 40, h: 28, notFlag: 'guardPassed' },        // niet onzichtbaar: je staat alleen bovenaan de trap (lopen kan niet)
        { x: 44, y: 212, w: 104, h: 40, requiresFlag: 'guardPassed' },  // onzichtbaar: de onderste treden van de trap
        { x: 36, y: 246, w: 444, h: 66, requiresFlag: 'guardPassed' }   // onzichtbaar: de natte kerkervloer, van de trap tot vlak voor de cel
      ],
      obstacles: [
        { x: 182, y: 200, w: 94, h: 28 },                // de houten tafel met de fles
        { x: 276, y: 196, w: 70, h: 28 },                // de tonnen naast de tafel
        { x: 36, y: 240, w: 456, h: 78, notFlag: 'guardPassed',
          msg: { nl: 'Je durft geen stap in de kerker te zetten — de reusachtige wachter zou je meteen zien. Alleen als je ONZICHTBAAR bent kun je verder. (De trap terug naar boven kan wel.)', en: 'You dare not set a foot into the dungeon — the huge guard would spot you at once. Only while INVISIBLE can you go on. (The stair back up still works.)' } }   // hele kerkervloer op slot tot je onzichtbaar bent — alleen de trap terug
      ],
      overlays: [],
      fx: {
        ...
        nutPlant:  { x: 91, y: 294, img: 'assets/art/plant-dance.png', imgNut: 'assets/art/item-bignut.png' },   // wondernoot + dansend plantje in het plasje links (2px lager)
        waterGlint: { x: 60, y: 285, w: 30 },                            // het plasje glinstert blauw
        lockGlint: { x: 418, y: 165, r: 9, col: '212,224,240', colOut: '200,214,235', twCol: '228,236,250', sparkCol: '245,248,255', requiresFlag: 'gotCellKey', doneFlag: 'fatherFreed' }   // het celslot glinstert ZILVER — kleiner, 2px hoger + 2px naar links
      },
      npcs: [
      ...
      hotspots: [
        ...
        },
        {
          ...
          walkTo: { x: 162, y: 252 },
          look: { nl: 'Zware roestige kettingen hangen aan de muur bij de trap. Beter om er vanaf te blijven — één tik en het galmt door de hele kerker.', en: 'Heavy rusty chains hang on the wall by the stairs. Better leave them alone — one touch and the clang echoes through the whole dungeon.' }
        },
        {
          id: 'puddle',
          ...
          look: (state) => state.flags.plantDancing
            ...
            ? { nl: 'De wondernoot dobbert in het plasje water. Nu nog een beetje magie om hem te laten groeien... je DANS-SPREUK misschien? (Tik hem aan in je tas.)', en: 'The wonder-nut bobs in the puddle. Now a little magic to make it grow... your DANCE SPELL perhaps? (Tap it in your bag.)' }
            : { nl: 'Tussen de stenen bij de trap glinstert een ondiep plasje kwelwater. Precies genoeg water om iets kleins in te laten groeien...', en: 'Between the stones by the stairs glints a shallow puddle of seep-water. Just enough water for something small to grow in...' },
          use: {
            bignut: {
              consume: 'bignut',
              setFlag: 'nutInPuddle',
              text: { nl: 'PLONS! De wondernoot dobbert rond in het plasje en het groene scheutje wipt nieuwsgierig omhoog. Hij wíl groeien... maar er is magie nodig. De dans-spreuk — “laat wat stil staat vrolijk dansen”! (Tik de DANS-SPREUK aan in je tas.)', en: 'SPLASH! The wonder-nut bobs about in the puddle and the little green sprout perks up curiously. It WANTS to grow... but it needs magic. The dance spell — “make what stands still dance”! (Tap the DANCE SPELL in your bag.)' }
            }
          }
        },
        {
        ...
        },
        {
          ...
          keyPuzzle: {
            ...
            hint: { nl: 'De GOUDEN sleutel past op de cel — maar hij zit KLEM onder de andere sleutels! Alleen de sleutel die aan de beurt is, komt los: schuif de stippen-sleutels opzij in volgorde — eerst •, dan ••, dan ••• — en pak dán de gouden. HOUD een sleutel VAST om hem te schuiven, en laat de GELUID-meter niet vollopen: pauzeer af en toe!', en: 'The GOLDEN key fits the cell — but it is STUCK beneath the other keys! Only the key whose turn it is will come loose: slide the dotted keys aside in order — • first, then ••, then ••• — and only then take the golden one. HOLD a key to slide it, and don’t let the NOISE meter fill up: pause now and then!' },
            keys: [
            ...
            doneText: { nl: 'De gouden celsleutel zit al in je tas.', en: 'The golden cell key is already in your bag.' }
          }
        },
        {
          id: 'father',
          ...
          look: (state) => state.flags.fatherFreed
            ...
            : (state.flags.gotNut || state.flags.guardTurned)
            ? { nl: 'Vader wacht gespannen achter de tralies. “Leid de wachter af met dat plantje, jongen — en steek dán de gouden sleutel in het slot!”', en: 'Father waits tensely behind the bars. “Distract the guard with that little plant, my boy — and THEN put the golden key in the lock!”' }
            : state.flags.gotCellKey
            ? { nl: 'Vader kijkt naar de gouden sleutel in je onzichtbare hand en glimlacht flauwtjes. “Goed zo, jongen. Nu de wachter nog... hij mag de zwevende sleutel niet zien.”', en: 'Father looks at the golden key in your invisible hand and smiles faintly. “Well done, my boy. Now the guard... he mustn’t see the floating key.”' }
            : { nl: 'Onzichtbaar sluip je naar de tralies. De magere man kijkt op — vermoeide ogen, rood-bruin haar net als dat van jou. “...Finn?” fluistert hij. “Mijn jongen! De wachter legt zijn sleutelBOS altijd op de tafel — de GOUDEN sleutel past op deze cel. Pak hem muisstil, jongen: onzichtbaar of niet, HOREN kan hij je nog wél...”', en: 'Invisible, you creep to the bars. The thin man looks up — tired eyes, red-brown hair just like yours. “...Finn?” he whispers. “My boy! The guard always leaves his key RING on the table — the GOLDEN key fits this cell. Take it quiet as a mouse, my boy: invisible or not, he can still HEAR you...”' },
          gift: {
            requiresFlag: ['gotCellKey'],
            notFlag: 'guardTurned',
            setFlag: 'gotNut',
            give: ['bignut'],
            text: { nl: 'Vader kijkt naar de starende wachter en grinnikt zacht. Hij haalt iets uit zijn zak en drukt het door de tralies in je hand: een grote, glanzende WONDERNOOT met een piepklein scheutje erin. “Van de tovenaar gekregen, lang geleden. Gooi hem in dat plasje water bij de trap, spreek je DANS-SPREUK uit... en let dan maar eens op de wachter.”', en: 'Father glances at the staring guard and chuckles softly. He takes something from his pocket and presses it through the bars into your hand: a big, glossy WONDER-NUT with a tiny sprout inside. “A gift from the wizard, long ago. Toss it into that puddle of water by the stairs, speak your DANCE SPELL... and then keep an eye on the guard.”' }
          },
          use: {
            cellkey: {
              requiresFlag: ['guardTurned'],
              requiresText: { nl: 'Nog niet! De wachter staart recht naar de cel — zelfs onzichtbaar zou hij de zwevende gouden sleutel zien. Vraag vader om raad: hij weet vast iets om de wachter af te leiden...', en: 'Not yet! The guard stares straight at the cell — even invisible, he would see the floating golden key. Ask father for advice: he surely knows a way to distract the guard...' },
              consume: 'cellkey',
              setFlag: 'fatherFreed',
              win: true,
